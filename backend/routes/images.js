...
router.post("/:_id", upload.single("file"), async (req, res) => {
    try {
        ...
        const product = await UsedItem.updateOne(
            { _id },
        ...
        if (!product)
            return res.json({
                message: "Error updating usedItem!",
                error: true,
            });

        res.json({ message: "Image successful saved!", image: savedImage, success: true });
    } catch (err) {
        console.log("Error on POST /images/:_id");
        res.json({ message: "Image: Something went wrong!", error: true });
        return;
    }
});
